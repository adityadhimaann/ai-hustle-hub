      ...
      <div className="container mx-auto px-6 lg:px-20 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center pointer-events-auto">
        ...
        <div className="flex flex-col gap-6 w-full lg:max-w-md ml-auto relative">
          {cards.map((card, i) => (
            <div
              key={i}
              className="glass-card gsap-fade-in opacity-0 p-8 rounded-2xl transform transition-all duration-[600ms] hover:scale-105 hover:-translate-y-2 cursor-pointer group relative overflow-hidden"
              style={{ transitionDelay: `${i * 150}ms` }}
            >
              <div className="absolute inset-0 bg-gradient-to-r from-[#00f0ff]/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none"></div>
              
              <div className="flex items-center gap-6 relative z-10">
                <div className="text-gray-500 group-hover:text-[#00f0ff] transition-colors duration-500">
                  {card.icon}
                </div>
                <div>
                  <h3 className="text-xl font-bold font-orbitron text-neutral-900 tracking-wide">{card.title}</h3>
                  <p className="text-neutral-700 text-sm leading-relaxed mt-1 font-inter">{card.desc}</p>
                </div>
              </div>
            </div>
          ))}
