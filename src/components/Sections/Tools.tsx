    ...
    <section id="tools" className="relative min-h-[140vh] flex flex-col justify-center py-24 pointer-events-none">
      <div className="container mx-auto px-6 lg:px-20 z-10 pointer-events-auto text-right">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-left w-full mx-auto">
          {tools.map((tool, i) => (
            <div key={i} className="gsap-fade-in opacity-0 glass-card p-8 rounded-2xl group flex flex-col justify-between hover:bg-black/5 transition-all duration-300 relative overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-[#00f0ff]/10 blur-[50px] mix-blend-screen opacity-0 group-hover:opacity-100 transition-opacity duration-700"></div>
              
              <div className="relative z-10">
                <h3 className="text-2xl font-bold font-orbitron text-neutral-900 mb-4 group-hover:text-[#00f0ff] transition-colors duration-300">{tool.name}</h3>
                <p className="text-neutral-700 font-inter mb-8">{tool.desc}</p>
              </div>

              <a href={tool.link} target="_blank" rel="noopener noreferrer" className="relative z-10 inline-flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-black hover:text-[#00f0ff] transition-colors w-max group/link">
